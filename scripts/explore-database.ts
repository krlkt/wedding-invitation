...
async function exploreDatabase() {
  try {
    ...
    for (const table of tables.rows) {
      ...
      console.log(`   ${table.name.padEnd(8)} | ${count.rows[0].count.toString().padStart(3)} records`);
    }
    ...
    for (const stat of rsvpStats.rows) {
      ...
      console.log(`   ${emoji} ${stat.attend.padEnd(6)} | ${stat.count.toString().padStart(3)} responses`);
    }
    ...
    console.log('\n🪑 Table Management:');
    const tableStats = await query<{location: string, tables: number, seated: number, total: number}>(`
      SELECT
        t.location,
    ...
    for (const stat of tableStats.rows) {
      console.log(`   ${stat.location.padEnd(10)} | ${stat.tables} tables, ${stat.seated}/${stat.total} seats filled`);
    }

    console.log('\n🎊 Total Wedding Data:');
    console.log(`   📧 ${groups.rows.length} invitation groups managed the guest list`);
    console.log(`   👥 ${(await query<{count: number}>('SELECT COUNT(*) as count FROM guests')).rows[0].count} total guests invited`);
    console.log(`   ✉️  ${(await query<{count: number}>('SELECT COUNT(*) as count FROM rsvp')).rows[0].count} RSVP invitations sent`);
    console.log(`   💌 ${(await query<{count: number}>('SELECT COUNT(*) as count FROM wish')).rows[0].count} heartfelt wishes received`);
    console.log(`   🪑 ${(await query<{count: number}>('SELECT COUNT(*) as count FROM tables')).rows[0].count} tables arranged for seating`);
