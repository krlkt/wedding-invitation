...
export function StartingSectionForm({
    ...
    const stableOnLocalChange = useCallback((draft: Partial<StartingSectionContent>) => {
        if (onLocalChange) {
            onLocalChange(draft)
        }
    }, [onLocalChange])

    const stableOnChangeTracking = useCallback((hasChanges: boolean, fields: Set<string>) => {
        if (onChangeTracking) {
    ...
    const handleUploadBackground = async () => {
        try {
            ...
            if (draftStartingSectionContent?.backgroundFilename || startingSectionContent?.backgroundFilename) {
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className={`space-y-2 rounded-lg transition-all ${changedFields.has('groomDisplayName') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                        ...
                        <div className={`space-y-2 rounded-lg transition-all ${changedFields.has('brideDisplayName') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                ...
                <div className="space-y-4">
                    ...
                    <div className={`flex items-center space-x-2 rounded-lg transition-all ${changedFields.has('showParentInfo') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                        <Checkbox
                            id="showParentInfo"
                            checked={showParentInfo}
                            onCheckedChange={(checked) =>
                                setValue('showParentInfo', checked as boolean)
                            }
                        />
                    ...
                    {showParentInfo && (
                        <div className="space-y-4 pl-4 border-l-2 border-gray-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className={`space-y-2 rounded-lg transition-all ${changedFields.has('groomFatherName') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                                    <Label htmlFor="groomFatherName">Groom&apos;s Father Name</Label>
                                    <Input {...register('groomFatherName')} id="groomFatherName" />
                                </div>

                                <div className={`space-y-2 rounded-lg transition-all ${changedFields.has('groomMotherName') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className={`space-y-2 rounded-lg transition-all ${changedFields.has('brideFatherName') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                                    <Label htmlFor="brideFatherName">Bride&apos;s Father Name</Label>
                                    <Input {...register('brideFatherName')} id="brideFatherName" />
                                </div>

                                <div className={`space-y-2 rounded-lg transition-all ${changedFields.has('brideMotherName') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                ...
                <div className="space-y-4">
                    ...
                    <div className={`flex items-center space-x-2 rounded-lg transition-all ${changedFields.has('showWeddingDate') ? 'ring-2 ring-yellow-400 bg-yellow-50 p-2' : ''}`}>
                        <Checkbox
                            id="showWeddingDate"
                            checked={showWeddingDate}
                            onCheckedChange={(checked) =>
                                setValue('showWeddingDate', checked as boolean)
                            }
                        />
                ...
                <div className="space-y-4">
                    ...
                    {(draftStartingSectionContent?.backgroundFilename || startingSectionContent?.backgroundFilename) && (
                        <div className="p-4 bg-gray-50 rounded-md border">
                            <p className="text-sm font-medium mb-1">Current Media:</p>
                            ...
                            {(draftStartingSectionContent?.backgroundFileSize || startingSectionContent?.backgroundFileSize) && (
                                <p className="text-sm text-gray-600">
                                    {formatFileSize(draftStartingSectionContent?.backgroundFileSize || startingSectionContent?.backgroundFileSize || 0)}
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    <div className="space-y-2">
                            ...
                            <div className="flex-1 relative">
                                ...
                                {selectedFile && !isUploading && (
                                    <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
                                        <span className="text-sm text-gray-600 bg-white px-2">
                        ...
                        <p className="text-xs text-gray-500">
                            Images: max 10 MB (JPEG, PNG, WebP, GIF) | Videos: max 50 MB (MP4,
                            WebM)
                        </p>
