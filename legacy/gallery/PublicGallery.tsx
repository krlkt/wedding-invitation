...
export default function WeddingGallery() {
  ...
  return (
    <div className="bg-white px-2 py-4 text-center">
      <h1 className="mb-4 text-center font-serif text-4xl font-semibold">Photos of us 📸</h1>
      <p className="mb-4">
        Click on the photo to open in fullscreen and swipe right/left to go next/previous
      </p>
      <Gallery>
        <div
          className={`grid gap-1 transition-all duration-300 ${isPortrait ? 'grid-cols-2' : 'grid-cols-3'}`}
        >
            ...
            <Item key={idx} original={src} thumbnail={src} width={width} height={height}>
              {({ ref, open }) => (
                <div
                  ref={ref}
                  onClick={open}
                  className="relative aspect-square cursor-pointer overflow-hidden rounded-xl shadow-sm"
                >
                  <Image
                    src={src}
